<script setup>
import {ref,onMounted} from 'vue'
function go(event){
        event.target.parentNode.classList.toggle('flipped');
        }
const words=[
// "She burned her hand.",
// "He hit his head on the door",
// "He broke his leg.",
// "It is bleeding",
// "It hurts a lot.",
// "Stick/put a plaster on the wound",
// "I hurt my knee.",
// "She cut her finger",
// "She takes care of her child.",
// "He has some bruises",
// "He has a black eye.",
// "He had a car crash",
// "He has a scar.",
// "She had a heart attack",
// "He has his leg in a cas",
"myself",
"She burned her hand.",
"head",
"cookie",
"He hit his head on the door.",
"He has some bruises.",
"find",
"He has a black eye.",
"He has a scar.",
"Please stick a plaster on the wound.",
"Did you hurt your back?",
"It sometimes happens.",
"plan",
"injury",
"She cut her finger.",
"to have an accident",
"fall down",
"school trips",
"I don’t like school trips.",
"That’s too bad!",
"Last summer I fell and broke my leg.",
"We plan to go to the lake.",
"Oh, does anyone have a plaster?",
"It hurts a lot.",
"take care",
"He broke his leg.",
"She takes care of her mother.",
"I learnt how to take care of myself.",
"Let me see.",
"He had a car crash.",
"It is bleeding.",
"back",
"Here you are.",
"I participated in Helal-e-Ahmar first aid classes.",
"I hurt my knee.",
"Can you give me some advice?",
"twist",
"He has his leg in a cast.",
"after",
"Just a second!",
"Do you want to come?",
"Please hurry up!",
"I stayed home for two weeks!",
"lake",
"leg",
"I didn’t know that.",
"She had a heart attack.",
"She takes care of her child.",
"happen",
"break",
"ankle",
"I twisted my ankle last winter.",
"cut",
"I don’t think so.",
];
        
const defs=[
" [ضمیر انعکاسی اول شخص مفرد]",
" او دستش را سوزاند.",
" سر",
" کلوچه",
" او سرش را به در زد.",
" او چند کبودی دارد.",
" پیدا کردن",
" او زیر چشمش کبود شده است.",
" او (جای) زخم دارد.",
" لطفا روی زخم چسب زخم بچسبانید.",
" به کمرت آسیب رسیده است؟",
" برخی اوقات پیش می آید.",
" برنامه‌ریزی کردن",
" صدمه",
" او انگشتش را برید.",
" تصادف کردن",
" (به زمین) افتادن",
" اردوهای مدرسه",
" من اردوهای مدرسه را دوست ندارم.",
" چقدر بد!",
" تابستان گذشته من افتادم و پایم شکست.",
" ما برنامه داریم به دریاچه برویم.",
" اوه، آیا کسی چسب زخم دارد؟",
" خیلی درد می کند.",
" مراقب بودن",
" او پایش شکست.",
" او از مادرش مراقبت می کند.",
" من یاد گرفتم چگونه از خودم مراقبت کنم.",
" بگذار ببینم.",
" او با اتومبیل تصادف کرد.",
" دارد خون می آید.",
" پشت",
" بفرمایید.",
" من در کلاس های کمک های اولیه هلال احمر شرکت کردم.",
" زانویم صدمه دیده است.",
" می توانید کمی من را نصیحت کنید؟",
" پیچ خوردن",
" او پایش را گچ گرفته است.",
" بعد از",
" (فقط) یک لحظه!",
" می خواهی بیایی؟",
" لطفا عجله کنید!",
" من دو هفته در خانه ماندم!",
" دریاچه",
" پا",
" من آن (موضوع) را نمی دانستم.",
" او سکته قلبی کرد.",
" او از کودکش مراقبت می کند.",
" اتفاق افتادن",
" شکستن",
" مچ پا",
" زمستان گذشته مچ پایم پیچ خورد.",
" بریدن",
" فکر نمی کنم.",
];

console.log(defs,words);
// const arr_front=[0,1,2,5,6,11,17,22,23,28,29,33,34,46,50];
const arr_front=[0];
// const arr_back=[2,5,6,11,17,22,23,28,29,33,34,46,50];
const arr_back=[0];
</script>

<template>
<!-- <div class="overflow-scroll bg-gradient-to-r from-green-300 via-blue-500 to-purple-600"> -->
<div class="overflow-scroll bg-gradient-to-tl from-sky-400 via-cyan-300 to-slate-100">

    <div class="cont w-[94vw]  pt-10 overflow-scroll flex flex-wrap flex-row-reverse justify-around h-[65vh]
   mx-auto gap-3">
  
    <div v-for="(item,index) in words"
         class="cards" 
         @click="go($event)">
          <!-- front -->
        <div class="card front text-black pt-[2vh]" v-if="!arr_front.includes(index)">{{item}}</div>    
        <div class="card front text-black pt-[2vh]" v-else>{{item}}</div>

          <!-- .back -->
        <div class="car.back text-black bg-blue-900 text-white pt-[2vh]" v-if="!arr_back.includes(index)">{{defs[index]}}</div>
        <div  class="car.back text-black pt-[2vh]" v-else>{{defs[index]}}</div>
</div>

</div>
<!-- <div class="animm h-[30vh] w-[100vw]" style="z-index:2">i am anim</div> -->
<RouterLink to="/paye7index">
      <!-- <div class="h-[30vh] w-[100vw] text-center bg-yellow-300" >بازگشت</div> -->
      <img class="h-[5vh] w-[10vw] fixed bottom-30 right-1" src="@/assets/back-arrow.png" />
</RouterLink> <br>
</div>
</template>

<style scoped>
.cont {
  perspective: 1400px;
}
.cards {
  position: relative;
  width:40vw;
  height: 25vw;
  cursor: pointer;
  transform-style: preserve-3d;
  transform-origin: center center;
  transition: transform 1s;
}

.flipped {
  transform: rotateY(-180deg);
}

.card {
  background-color: black;
  background-image: url(@/assets/back1.jpg);
  background-size: 100%;
  background-position: center center;
  color: black;
  position: absolute;

  line-height: 30vw;
  color: black;
  font-size: 13px;
  text-align: center;
  backface-visibility: hidden;
  box-shadow: 1px 1px 5px 1px black;
}

.front {
  padding-top: 4vh;
  color: black;
  font-family: BalsamiqSansb;
  line-height: 2vh;
}

.back {
  /*color: blue;*/
  transform: rotateY(180deg);
  font-family: iranb;
  line-height: 2vh;
  padding-top: 4vh;
}

</style>
